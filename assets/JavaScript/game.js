$(document).ready(function() {
	...
	for (var i = 19; i < 120; i++) {     //random number to match between 19 and 120
		ranNum.push(i);
	}  //end for random

	for (var j = 1; j < 12; j++) {      //random number for crystals between 1 and 12
		crystals.push(j);
	}  //end for crystals
	...
	$(".gem").on("click", function() {         //wins
		...
		if (totalScore == randNumber) {       //total score win
			...
			$("#wins").html("Wins: " + wins);
			$("#endScreen").toggle();
			$("h2").html("CONGRATULATIONS!!!YOU WIN!");			
